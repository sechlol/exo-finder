...
def io_work(param: int, delay: float = 0.01) -> int:
    """IO-bound work simulation"""
    time.sleep(delay)
    return param * 2


def io_work_batch(params: list[int], delay: float = 0.01) -> list[int]:
    """IO-bound batch work simulation"""
    time.sleep(delay)  # Simulate a single IO operation for the batch
...
def run_parallel_execution(func, params, n_jobs, batch_size=None, task_profile=None, show_combined_progress=False):
    ...
    start = timer()
    results = list(parallel_execution(
        func=func,
        params=params,
        n_jobs=n_jobs,
...
def benchmark_batch_vs_non_batch(n_items=1000, n_jobs=4, n_runs=3):
    ...
    for task_type in ["CPU", "IO"]:
        ...
        if task_type == "CPU":
            ...
            complexity = 0.1  # Reduce complexity for benchmarking
            single_func_args = {"complexity": complexity}
            batch_func_args = {"complexity": complexity}
        else:
            single_func = io_work
            batch_func = io_work_batch
            delay = 0.01
            single_func_args = {"delay": delay}
            batch_func_args = {"delay": delay}
...
def main():
    ...
    # Save results to CSV
    batch_results.to_csv("benchmark_batch_results.csv", index=False)
    progress_results.to_csv("benchmark_progress_results.csv", index=False)
    print("\nResults saved to CSV files: benchmark_batch_results.csv and benchmark_progress_results.csv")
